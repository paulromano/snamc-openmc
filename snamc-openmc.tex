...
OpenMC uses a free gas approximation \cite{anl-gelbard-1979} wherein the
...
all dependencies, e.g. MPI, are satisfied, and the user is spared the trouble of
building the code.
...
Another important difference between OpenMC and many other transport codes is
that the input is divided into multiple files rather than one file. The
following files are required for every simulation:
\begin{itemize}
...
by the Consortium for Advanced Simulation of Light Water Reactors, an Energy
