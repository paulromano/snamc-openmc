...
\ead{paul.romano@alum.mit.edu}
\cortext[cor1]{Corresponding author. Tel.: +1 518 395 3010.}

\author[mit]{Nicholas E. Horelik}
\ead{nhorelik@mit.edu}

\author[mit]{Bryan R. Herman}
\ead{bherman@mit.edu}

\author[umich]{Adam G. Nelson}
\ead{nelsonag@umich.edu}
...
An indexing technique \citep{ane-leppanen-2009} based on pointers is used to
...
OpenMC uses a free gas approximation \citep{mc-sutton-2009} wherein the
...
\citep{nse-levitt-1972}. Probability tables are included in the ACE data for many
nuclides.

The method of successive generations \citep{nukleonik-lieberoth-1968} is used to
...
tally random variables \citep{physor-kelly-2012}. Like MCNP, OpenMC keeps track
...
utility is provided to convert voxel files into either SILO \citep{silo-2010} or
VTK \citep{vtk-2010} files that can be viewed in many well-established 3D data
...
\begin{figure}[htb]
  ...
  \caption{3D plot of a grid spacer in the BEAVRS benchmark
    \citep{mc-horelik-2013} as shown in ParaView.}
